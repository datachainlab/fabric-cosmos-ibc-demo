...
# This script is a copy of [this script](https://github.com/cosmos/relayer/blob/14195ec4c8c19f86a3b6bbe3750f392e436a5879/scripts/one-chain)
...
sleep 1
$BINARY --home $CHAINDIR/$CHAINID keys add relayer --keyring-backend="test" --output json > $CHAINDIR/$CHAINID/relayer_seed.json 2> /dev/null
sleep 1
$BINARY --home $CHAINDIR/$CHAINID keys add importer --keyring-backend="test" --output json > $CHAINDIR/$CHAINID/importer_seed.json 2> /dev/null
sleep 1
$BINARY --home $CHAINDIR/$CHAINID keys add exporter --keyring-backend="test" --output json > $CHAINDIR/$CHAINID/exporter_seed.json 2> /dev/null
sleep 1
$BINARY --home $CHAINDIR/$CHAINID add-genesis-account $($BINARY --home $CHAINDIR/$CHAINID keys --keyring-backend="test" show relayer -a) $coins &> /dev/null
sleep 1
$BINARY --home $CHAINDIR/$CHAINID add-genesis-account $($BINARY --home $CHAINDIR/$CHAINID keys --keyring-backend="test" show importer -a) $coins &> /dev/null
sleep 1
$BINARY --home $CHAINDIR/$CHAINID add-genesis-account $($BINARY --home $CHAINDIR/$CHAINID keys --keyring-backend="test" show exporter -a) $coins &> /dev/null
sleep 1
...
sleep 1
$BINARY --home $CHAINDIR/$CHAINID gentx validator --keyring-backend="test" --chain-id $CHAINID &> /dev/null
sleep 1
